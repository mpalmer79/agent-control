...
// Sign-in route. When Clerk is configured the Clerk SignIn widget is rendered.
...
export default async function SignInPage() {
  ...
  return (
    <div className="flex min-h-screen flex-col items-center justify-center gap-4 p-6 text-center">
      <h1 className="text-2xl font-semibold">Sign in to {PRODUCT.name}</h1>
      <p className="max-w-md text-sm text-muted-foreground">
        Authentication is provided by Clerk. Configure Clerk environment
        variables to enable sign in. The control plane shell is available for
